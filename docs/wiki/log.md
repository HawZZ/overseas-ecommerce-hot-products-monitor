...
## 2026-08-03 Shopee 台湾 CDP 采集器

- 新增配对、受限上传、类目映射、客户端撤销与批次状态接口。
- 新增 macOS 可见 Chrome/CDP 客户端和 12 小时 launchd 安装命令；不采集 Cookie、请求头或完整原始响应，不实现验证码或封禁绕过。
- 台湾榜单分为热销与同类目综合顺序，缺失历史点保持为空值。
