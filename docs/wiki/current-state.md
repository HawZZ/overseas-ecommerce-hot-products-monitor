...
- Shopee 台湾通过用户 macOS 的可见 Chrome/CDP 客户端采集公开页面样本；配对 Token 只有 `shopee-tw:ingest` 权限，服务端仅保存哈希，原始响应不落盘。
...
1. **台湾采集依赖用户电脑**：Chrome Profile 未初始化、电脑关机或页面要求人工验证时不会产生新批次；看板只保留并标记最后一次真实数据。
