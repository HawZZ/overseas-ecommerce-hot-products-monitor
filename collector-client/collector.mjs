...
async function init() {
  ...
  const config = { apiBase, cdpHost: "127.0.0.1", cdpPort: Number(argument("--port", "9222")), profileDir, clientName: argument("--name", "Shopee TW CDP") };
...
async function pair() {
  ...
  const response = await fetch(`${config.apiBase}/api/collector/pair`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code, name: config.clientName }) });
  const body = await response.json();
  if (!response.ok || !body.token) throw new Error(`配对失败：${body.error || response.status}`);
  await writePrivate(configPath, { ...config, clientId: body.clientId, token: body.token, tokenExpiresAt: body.expiresAt });
...
async function run() {
  ...
  try {
    const mapResponse = await fetch(`${config.apiBase}/api/collector/shopee-tw/category-map`, { headers: { Authorization: `Collector ${config.token}` } });
    ...
    const response = await fetch(`${config.apiBase}/api/collector/shopee-tw/batches`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Collector ${config.token}` }, body: JSON.stringify({ schemaVersion: "shopee-tw-cdp-v1", cycleId: crypto.randomUUID(), collectedAt: new Date().toISOString(), market: "TW", products: [...byKey.values()] }) });
